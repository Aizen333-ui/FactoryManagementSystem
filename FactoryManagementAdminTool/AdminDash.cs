    ...
    public partial class AdminDash : UserControl
    {
        ...
        private void LoadDashboardData()
        {
            try
            {
                ...
                DataTable disabled =
                    DBHelper.ExecuteDataTable(
                    "SELECT COUNT(*) AS Total FROM Users WHERE IsActive = 0",
                    null);


                lblDisabledUsers.Text =
                    disabled.Rows[0]["Total"].ToString();
                // System Admins
                DataTable admins =
                    DBHelper.ExecuteDataTable(
                        "SELECT COUNT(*) AS Total FROM SystemAdmins",
                        null);

                lblSystemAdmin.Text =
                    admins.Rows[0]["Total"].ToString();
